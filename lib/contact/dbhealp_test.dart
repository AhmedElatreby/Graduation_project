...
class DBHelper {
  ...
  Future<PersonalContactss> add(PersonalContactss contacts) async {
  ...
  Future<List<PersonalContactss>> getContacts() async {
    final  dbClient = await db;
    List<Map> maps =
    await dbClient.query('contacts', columns: ['id', 'name', 'contactNo']);
    final List<PersonalContactss> contacts = [];
    if (maps.isNotEmpty) {
      for (int i = 0; i < maps.length; i++) {
        contacts.add(PersonalContactss.fromMap(maps[i]));
      }
    }
    print(maps);
    return contacts;
  ...
  Future<int> update(PersonalContactss contacts) async {
    var dbClient = await db;
